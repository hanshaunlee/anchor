...
-- Helper: current user's household_id (from users table)
...
CREATE POLICY households_select ON households
  FOR SELECT USING (id = auth.user_household_id());
CREATE POLICY households_update ON households
  FOR UPDATE USING (id = auth.user_household_id());

-- Users: user can read users in same household
CREATE POLICY users_select ON users
  FOR SELECT USING (household_id = auth.user_household_id());
...
CREATE POLICY events_select ON events
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM sessions s WHERE s.id = events.session_id AND s.household_id = auth.user_household_id()
    )
  );

-- Events insert: allow for device belonging to household (typically via service role or device JWT)
-- For device uploads we use service_role or a dedicated device policy: insert where device_id in (select id from devices where household_id = auth.user_household_id())
  ...
  FOR INSERT WITH CHECK (
    EXISTS (
      ...
      WHERE s.id = events.session_id
        AND s.household_id = auth.user_household_id()
    )
  );
...
CREATE POLICY utterances_all ON utterances
  FOR ALL USING (
    EXISTS (SELECT 1 FROM sessions s WHERE s.id = utterances.session_id AND s.household_id = auth.user_household_id())
  );
...
CREATE POLICY mentions_all ON mentions
  FOR ALL USING (
    EXISTS (SELECT 1 FROM sessions s WHERE s.id = mentions.session_id AND s.household_id = auth.user_household_id())
  );
...
CREATE POLICY device_sync_state_select ON device_sync_state
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM devices d WHERE d.id = device_sync_state.device_id AND d.household_id = auth.user_household_id())
  );
...
CREATE POLICY session_embeddings_all ON session_embeddings
  FOR ALL USING (
    EXISTS (SELECT 1 FROM sessions s WHERE s.id = session_embeddings.session_id AND s.household_id = auth.user_household_id())
  );
