"""
Layer A: Rule-based motif detection for human-legible explanation tags.
Motifs: new_contact+urgency, bursty_contact, device_switch, contact→intent(sensitive)→payee cascade.
Keyword sets are configurable via config.graph or passed in to avoid hardcoding.
"""
...
def extract_motifs(
    ...
    """
    Returns (motif_tags: list[str], timeline_snippet: list[dict]).
    motif_tags are plain-English labels; timeline_snippet is 3–6 key events for UI.
    """
    ...
    return motif_tags_dedup, timeline_snippet
