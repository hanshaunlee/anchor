...
# Severity threshold for consent-gated escalation draft
DEFAULT_ESCALATION_SEVERITY_THRESHOLD = 4
...
def _normalize(household_id: str, events: list[dict]) -> tuple[list, list, list, list]:
    """Task 1b: Normalize to utterances, entities, mentions, relationships (GraphBuilder)."""
...
def _detect_risk_patterns(
    ...
    # Optional model layer: GNN (no checkpoint = skip)
    ...
    try:
        ...
        data = build_hetero_from_tables(
            "", sessions_for_graph, utterances, entities, mentions, relationships
        )
        ...
        if ckpt_path.exists():
            ...
            model.eval()
            risk_list, _ = run_inference(model, data, torch.device("cpu"), explain_node_idx=None)
            for r in risk_list:
    ...
    return risk_scores, motif_tags_global, timeline_snippet[:6], evidence_subgraph
...
def run_financial_security_playbook(
    ...
    dry_run: bool = False,
    escalation_severity_threshold: int = DEFAULT_ESCALATION_SEVERITY_THRESHOLD,
    persist_score_min: float = 0.3,
    ...
    consent_share = consent_state.get(CONSENT_SHARE_KEY, True)
    consent_watchlist = consent_state.get(CONSENT_WATCHLIST_KEY, True)
    ...
    utterances, entities, mentions, relationships = _normalize(household_id, events)
